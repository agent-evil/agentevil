...
The site is implemented and runs locally with `bun run dev`. Release setup began on September 22 at the owner's explicit request: the public repository https://github.com/agent-evil/agentevil and the Cloudflare Pages Direct Upload project `agentevil` now exist, and local Git tracks `main`. `CLOUDFLARE_ACCOUNT_ID` is configured in GitHub. The CI API token, first push, production deployment and domain activation are pending; live giscus comments remain optional and disconnected.
...
- The September 22 SEO and keyboard updates have passed type checking, build, static audit, 20 browser groups, 120 accessibility/Tab groups and state tests. A final six-group shortcut/focus regression also passed. All 12 Lighthouse pages passed >=95 in all four categories. See docs/VERIFICATION.md; do not infer production deployment from local tests.
...
- Static audit (2026-09-22): 111 HTML pages, 3,421 internal references, 108 sitemap URLs and 21 localized social images passed. Checks cover per-page unique descriptions, social fields and image files, indexing policy, hreflang destinations, Article body language, robots, llms links and feed counts. Conservative per-page gzip JS ceiling is 15,244 bytes, including inline scripts. The 169-file static build has a largest asset of 418,006 bytes, within Pages Free asset limits.
...
- Measured Lighthouse (2026-09-22, before the remaining article bodies were translated): all 12 representative mobile pages passed. Performance 96–100; accessibility, best practices and SEO all 100. Covers three homepages, article index, four article routes, Atlas index/detail, About and incident. In that run, `/fr/posts/think-for-yourself/` still served an English body. Local production preview, not field data. Exact scores and commands are in docs/VERIFICATION.md; raw reports are in test-results/lighthouse/.
...
## Release configuration and remaining external setup

- Git is initialized on `main` with origin https://github.com/agent-evil/agentevil. The public repository and Pages Direct Upload project `agentevil` have been created. The project production branch is `main`. No first deployment has succeeded yet.
- `CLOUDFLARE_ACCOUNT_ID` is set. `CLOUDFLARE_API_TOKEN` still requires a durable token with Account → Cloudflare Pages → Edit. The local Wrangler OAuth session can manage Pages but cannot create account tokens or edit DNS. `agentevil.com` is an active zone in the same account. Never request credentials in chat.
- Optional public giscus IDs: see SETUP-GISCUS.md and .env.example. CI passes the four public GitHub repository variables to the build.
- GitHub Actions now gates PRs and publishing on static validation, browser/state tests, keyboard/axe checks and a 12-page Lighthouse matrix (all four scores >=95), using the same uploaded dist artifact. It retains verification reports even on failure. No workflow has actually run remotely.
- Real giscus login/posting remains unconfigured. Cloudflare production behavior and domain activation still need live verification.
...
All six slugs have exactly one EN, ZH and FR body. Paragraph/heading counts, four Evil notes per article, publication dates, Atlas references and sources match across each translation set; the dialogue article has 24 speeches in every language. No missing section was found in the source review. Static validation now rejects missing or duplicate language bodies instead of accepting a fallback. Latest release checks: Astro check 0 errors/warnings/hints; build 111 HTML pages; audit 3,413 internal references, 108 sitemap URLs, 21 localized social images and 15,118 gzip bytes maximum page JavaScript. Full remote release gates are pending.
